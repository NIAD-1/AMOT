...
export class EvidenceService {
  static async generateUploadUrl(data: { fileName: string; mimeType: string }) {
    ...
    const url = `${env.S3_ENDPOINT || 'https://storage.googleapis.com'}/${env.S3_BUCKET || 'amot-vault'}/${key}`;
    return { url, key };
  }
  ...
  static async getById(id: number) {
    ...
    const downloadUrl = `${env.S3_ENDPOINT || 'https://storage.googleapis.com'}/${env.S3_BUCKET || 'amot-vault'}/${evidence.storageKey}`;
